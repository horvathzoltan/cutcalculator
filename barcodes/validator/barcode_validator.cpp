                                        ...
                                        CsvImporter::FileContext& ctx)
{
    ...
    // 2) Globális uniqueness check a registryben
    if (!br.isBarcodeUnique(trimmedCode)) {
        ctx.addError(ctx.currentLineNumber(),
                     QString("Barcode collision: %1").arg(trimmedCode),
                     trimmedCode,
                     name);
        zEventERROR(QString("Barcode collision: %1 (%2)").arg(trimmedCode, entityType));
        return false;
    }
    ...
    if (!br.registerNew(trimmedCode, entityType, id, name)) {
        ...
        ctx.addError(ctx.currentLineNumber(),
                     QString("Failed to register barcode: %1").arg(trimmedCode),
                     trimmedCode,
                     name);
        zEventERROR(QString("Failed to register barcode: %1 (%2)")
                        .arg(trimmedCode, entityType));
        return false;
    }
...
void BarcodeValidator::retire(const QString& code, const QString& reason) {
    ...
    if (!br.retire(trimmedCode, reason)) {
        zEventWARN(QString("Retire requested for unknown or already retired code: %1")
                       .arg(trimmedCode));
    }
}
